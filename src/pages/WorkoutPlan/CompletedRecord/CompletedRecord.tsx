...
const CompletedRecord: React.FC<CompletedRecordProps> = () => {
    ...
    /** Edit note */
    // TBC
    const [isShowEditNote, setShowEditNote] = useState(false);
    ...
    const handleSaveEditNote = async (value: string) => {
        if (recordId && value !== record.note) {
            await ExerciseService.updateRecord(recordId, { note: value });

            setRecord((preState) => ({ ...preState, note: value }));
        }
    ...
    return (
        <div>
            ...
            <div className='relative'>
                <img
                    src={CelebrateBg}
                    alt='celebrate-background'
                    className='absolute top-0 left-0 w-full'
                />
                <div className='relative z-10 flex flex-col items-center p-4'>
                    <img src={BodyPartBadge} alt='body-part-badge' className='mb-2' />
                    <h2 className='text-body-bold-l'>{t('subtitle')}</h2>
                    <div className='text-body-s text-tertiary mb-2'>
                        {moment(record.time).format('YYYY/MM/DD')}
                    </div>
                    {record.note ? (
                        <div className='border border-secondary w-full rounded-lg bg-primary flex justify-between'>
                            <div className='overflow-hidden py-4 pl-4'>
                                <div className='text-body-bold-m text-secondary mb-1'>
                                    {t('note-title')}
                                </div>
                                <p className='text-body-s text-tertiary whitespace-nowrap text-ellipsis overflow-hidden'>
                                    {record.note}
                                </p>
                            </div>
                        ...
                        <HCButton color='primary' onClick={handleClickNoteBtn}>
            ...
            <div className='flex flex-col gap-y-4 p-4'>
                {record.exerciseList.map((exercise) => (
                        ...
                        <div className='py-2'>
                            {exercise.sets.map((set, setIndex) => (
                                <div className='flex items-center text-body-s  px-4 py-2'>
                                    <div className='text-secondary grow'>
                                        Set {setIndex + 1 > 9 ? setIndex + 1 : `0${setIndex + 1}`}
                                    </div>
                                    <div className='text-primary'>{set.reps} reps</div>
                                    <div className='text-primary min-w-[58px] text-right'>
                                        {set.weight} kg
                                    </div>
                                </div>
                            ))}
