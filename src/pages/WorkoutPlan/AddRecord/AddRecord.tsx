...
import type { WorkoutRecord, WorkoutRecordExerciseSetData, WorkoutRecordExerciseData } from '~/services/exercise';
...
const AddRecord: React.FC<AddRecordProps> = () => {
    ...
    setRecord(
      {
        ...
        exerciseList: plan.exerciseList.map((item) => {
          ...
          const recordSets: WorkoutRecordExerciseSetData[] = Array(sets).fill(null).map(() => ({
            reps,
            weight: 0,
            id: uuidv4()
          }));
  ...
  useEffect(() => {
    const transExerciseFromTemplate = ({
      ...
      const exercise = exerciseList.find((exercise) => exercise.id === exerciseId) as Exercise;
      ...
      return {
        ...
        description: `${level[exercise.level][language]} · ${ExerciseService.getExerciseMusclesText(
          exercise.muscles,
        )}`,
        isCompleted: false,
      };
  ...
  const handleConfirmAddExercise = (value: AddExerciseValue) => {
    ...
    setRecord({
      ...record,
      exerciseList: [...record.exerciseList, { id, exerciseId, sets: Array(sets).fill(null).map(() => ({ reps, weight: 0, id: uuidv4() })) }]
    });
    ...
    <Layout
        ...
        >
          ...
          <HCHeaderRegion behavior='fully' className='bg-primary px-4'>
            ...
            <div className='text-body-xs'>
              <span className='text-placeholder'>{t('completeness')}: {percentage}%</span>
      ...
      footer={
        <div className="p-4 w-full border border-secondary bg-primary">
          <HCButton color="primary" disabled={isDisabledSave} onClick={handleSaveRecord}>{t('end-of-record')}</HCButton>
        </div>
      }
    >
      <HCWorkoutForm data={workoutFormData} setData={setWorkoutFormData} />

      <AddExercise isShow={isShowAddExercise} type={ADD_EXERCISE_TYPE.RECORD} onClose={handleCloseAddExercise} onConfirm={handleConfirmAddExercise} />
    </Layout>
  );
};
