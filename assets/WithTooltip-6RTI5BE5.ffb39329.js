import { _ as __commonJS, a as __toESM, b as _inheritsLoose, d as _objectWithoutPropertiesLoose, f as _extends, m as memoize, e as emotion_styled_browser_esm_default, l as lightenColor, h as createPopper } from "./DocsRenderer-QMFTTPNP.832b1216.js";
...
var ManagerReferenceNodeContext = react.exports.createContext(), ManagerReferenceNodeSetterContext = react.exports.createContext();
function Manager(_ref) {
  var children = _ref.children, _React$useState = react.exports.useState(null), referenceNode = _React$useState[0], setReferenceNode = _React$useState[1], hasUnmounted = react.exports.useRef(false);
  ...
  return react.exports.createElement(ManagerReferenceNodeContext.Provider, { value: referenceNode }, react.exports.createElement(ManagerReferenceNodeSetterContext.Provider, { value: handleSetReferenceNode }, children));
}
var unwrapArray = function(arg) {
...
}, useIsomorphicLayoutEffect = typeof window < "u" && window.document && window.document.createElement ? react.exports.useLayoutEffect : react.exports.useEffect;
var import_react_fast_compare = __toESM(require_react_fast_compare());
var EMPTY_MODIFIERS = [], usePopper = function(referenceElement, popperElement, options) {
  options === void 0 && (options = {});
  var prevOptions = react.exports.useRef(null), optionsWithDefaults = { onFirstUpdate: options.onFirstUpdate, placement: options.placement || "bottom", strategy: options.strategy || "absolute", modifiers: options.modifiers || EMPTY_MODIFIERS }, _React$useState = react.exports.useState({ styles: { popper: { position: optionsWithDefaults.strategy, left: "0", top: "0" }, arrow: { position: "absolute" } }, attributes: {} }), state = _React$useState[0], setState = _React$useState[1], updateStateModifier = react.exports.useMemo(function() {
  ...
  }, []), popperOptions = react.exports.useMemo(function() {
  ...
  }, [optionsWithDefaults.onFirstUpdate, optionsWithDefaults.placement, optionsWithDefaults.strategy, optionsWithDefaults.modifiers, updateStateModifier]), popperInstanceRef = react.exports.useRef();
...
function Popper(_ref) {
  var _ref$placement = _ref.placement, placement = _ref$placement === void 0 ? "bottom" : _ref$placement, _ref$strategy = _ref.strategy, strategy = _ref$strategy === void 0 ? "absolute" : _ref$strategy, _ref$modifiers = _ref.modifiers, modifiers = _ref$modifiers === void 0 ? EMPTY_MODIFIERS2 : _ref$modifiers, referenceElement = _ref.referenceElement, onFirstUpdate = _ref.onFirstUpdate, innerRef = _ref.innerRef, children = _ref.children, referenceNode = react.exports.useContext(ManagerReferenceNodeContext), _React$useState = react.exports.useState(null), popperElement = _React$useState[0], setPopperElement = _React$useState[1], _React$useState2 = react.exports.useState(null), arrowElement = _React$useState2[0], setArrowElement = _React$useState2[1];
  react.exports.useEffect(function() {
  ...
  }, [placement, strategy, onFirstUpdate, modifiers, arrowElement]), _usePopper = usePopper(referenceElement || referenceNode, popperElement, options), state = _usePopper.state, styles = _usePopper.styles, forceUpdate = _usePopper.forceUpdate, update = _usePopper.update, childrenProps = react.exports.useMemo(function() {
    return { ref: setPopperElement, style: styles.popper, placement: state ? state.placement : placement, hasPopperEscaped: state && state.modifiersData.hide ? state.modifiersData.hide.hasPopperEscaped : null, isReferenceHidden: state && state.modifiersData.hide ? state.modifiersData.hide.isReferenceHidden : null, arrowProps: { style: styles.arrow, ref: setArrowElement }, forceUpdate: forceUpdate || NOOP, update: update || NOOP_PROMISE };
  }, [setPopperElement, setArrowElement, placement, state, styles, update, forceUpdate]);
  return unwrapArray(children)(childrenProps);
}
var import_warning = __toESM(require_warning());
function Reference(_ref) {
  var children = _ref.children, innerRef = _ref.innerRef, setReferenceNode = react.exports.useContext(ManagerReferenceNodeSetterContext), refHandler = react.exports.useCallback(function(node) {
...
var TooltipContext = React.createContext({}), callAll = function() {
  ...
  return _proto.componentDidMount = function() {
    ...
    return React.createElement(TooltipContext.Provider, { value: this.contextValue }, tooltip({ arrowRef: arrowProps.ref, getArrowProps: this.getArrowProps, getTooltipProps: this.getTooltipProps, placement, tooltipRef: this.getTooltipRef }));
...
var DEFAULT_MUTATION_OBSERVER_CONFIG = { childList: true, subtree: true }, TooltipTrigger = function(_Component) {
  ...
  return _proto.componentWillUnmount = function() {
    ...
    var _this2 = this, _this$props = this.props, children = _this$props.children, tooltip = _this$props.tooltip, placement = _this$props.placement, trigger = _this$props.trigger, getTriggerRef = _this$props.getTriggerRef, modifiers = _this$props.modifiers, closeOnReferenceHidden = _this$props.closeOnReferenceHidden, usePortal = _this$props.usePortal, portalContainer = _this$props.portalContainer, followCursor = _this$props.followCursor, getTooltipRef = _this$props.getTooltipRef, mutationObserverOptions = _this$props.mutationObserverOptions, restProps = _objectWithoutPropertiesLoose(_this$props, ["children", "tooltip", "placement", "trigger", "getTriggerRef", "modifiers", "closeOnReferenceHidden", "usePortal", "portalContainer", "followCursor", "getTooltipRef", "mutationObserverOptions"]), popper = React.createElement(Popper, _extends({ innerRef: getTooltipRef, placement, modifiers: [{ name: "followCursor", enabled: followCursor, phase: "main", fn: function(data) {
      ...
      return React.createElement(Tooltip, _extends({ arrowProps, closeOnReferenceHidden, isReferenceHidden, placement: placement2, update, style, tooltip, trigger, mutationObserverOptions }, { clearScheduled: _this2.clearScheduled, hideTooltip: _this2.hideTooltip, innerRef: ref }));
    });
    return React.createElement(Manager, null, React.createElement(Reference, { innerRef: getTriggerRef }, function(_ref4) {
      var ref = _ref4.ref;
      return children({ getTriggerProps: _this2.getTriggerProps, triggerRef: ref });
    }), this.getState() && (usePortal ? reactDom.exports.createPortal(popper, portalContainer) : popper));
...
var match = memoize(1e3)((requests, actual, value, fallback = 0) => actual.split("-")[0] === requests ? value : fallback), ArrowSpacing = 8, Arrow = emotion_styled_browser_esm_default.div({ position: "absolute", borderStyle: "solid" }, ({ placement }) => {
  let x = 0, y = 0;
  switch (true) {
  ...
  return { transform: `translate3d(${x}px, ${y}px, 0px)` };
          ...
          `, borderRadius: theme.appBorderRadius * 2, fontSize: theme.typography.size.s1 } : {}), Tooltip2 = ({ placement, hasChrome, children, arrowProps, tooltipRef, arrowRef, color, ...props }) => React.createElement(Wrapper, { hasChrome, placement, ref: tooltipRef, ...props, color }, hasChrome && React.createElement(Arrow, { placement, ref: arrowRef, ...arrowProps, color }), children);
Tooltip2.defaultProps = { color: void 0, arrowRef: void 0, tooltipRef: void 0, hasChrome: true, placement: "top", arrowProps: {} };
var { document: document2 } = scope, TargetContainer = emotion_styled_browser_esm_default.div`
...
`, WithTooltipPure = ({ svg, trigger, closeOnClick, placement, modifiers, hasChrome, tooltip, children, tooltipShown, onVisibilityChange, ...props }) => {
  ...
  return React.createElement(react_popper_tooltip_default, { placement, trigger, modifiers, tooltipShown, onVisibilityChange, tooltip: ({ getTooltipProps, getArrowProps, tooltipRef, arrowRef, placement: tooltipPlacement }) => React.createElement(Tooltip2, { hasChrome, placement: tooltipPlacement, tooltipRef, arrowRef, arrowProps: getArrowProps(), ...getTooltipProps() }, typeof tooltip == "function" ? tooltip({ onHide: () => onVisibilityChange(false) }) : tooltip) }, ({ getTriggerProps, triggerRef }) => React.createElement(Container, { ref: triggerRef, ...getTriggerProps(), ...props }, children));
};
...
var WithToolTipState = ({ startOpen = false, onVisibilityChange: onChange, ...rest }) => {
  let [tooltipShown, setTooltipShown] = react.exports.useState(startOpen), onVisibilityChange = react.exports.useCallback((visibility) => {
    onChange && onChange(visibility) === false || setTooltipShown(visibility);
  }, [onChange]);
  return react.exports.useEffect(() => {
    let hide = () => onVisibilityChange(false);
    ...
    return iframes.forEach((iframe) => {
      ...
      document2.removeEventListener("keydown", hide), unbinders.forEach((unbind) => {
        unbind();
      });
    };
  }), React.createElement(WithTooltipPure, { ...rest, tooltipShown, onVisibilityChange });
};
export {
  WithToolTipState,
...
//# sourceMappingURL=WithTooltip-6RTI5BE5.ffb39329.js.map
